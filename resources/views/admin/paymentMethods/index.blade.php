...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-8">
            {{-- <h1 class="page-header">PaymentMethods</h1> --}}
        </div>
        <div class="col-lg-4">
                ...
                <div class="panel-body">
                    ...
                    <div class="dataTable_wrapper">
                                ...
                                @isset($paymentMethods)
                                    @foreach($paymentMethods as $paymentMethod)
                                        <tr class="odd gradeX">
                                            ...
                                            <?php
                                                if($paymentMethod->is_activate == 1){$activate = '<span class="badge badge-info">active</span>';}
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            <li>
                                                                <a href="{{route('admin/paymentMethods/edit', $paymentMethod->id)}}" style="text-decoration: none; color: white; width: 75px;margin:auto" class="btn btn-success">
                                                                    edit
                                                                </a>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li >
                                                                <button class="dropdown-item btn btn-danger openDeleteFrom" data-toggle="modal" data-target="#myModalDelete" data-id="{{$paymentMethod->id}}">
                                                                    delete
                                                                </button>
                                                            </li>
                                                            <li class="divider"></li>
                                                            <li>
                                                                <button class="dropdown-item btn btn-priamry openActivationFrom" data-toggle="modal" data-target="#myModalActivation" data-id="{{$paymentMethod->id}}">
                                                                    activation
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\PaymentMethod::unArchive()->count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/paymentMethods/delete'))}}" method="get">
                                        {{ csrf_field() }}
                    ...
                    <div class="modal fade" id="myModalActivation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/paymentMethods/activate'))}}" method="get">
                                        {{ csrf_field() }}
                                        <p>Are You Sure To Update This Record ?</p>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/paymentMethods/pagination")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    edit_route =  "{{ route('admin/paymentMethods/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr>
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/paymentMethods/search') }}";
