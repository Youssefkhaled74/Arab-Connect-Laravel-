                ...
                <div class="panel-body">
                    ...
                    <div class="dataTable_wrapper">
                                ...
                                @isset($categories)
                                    @foreach($categories as $category)
                                        <tr class="odd gradeX">
                                            ...
                                            <td>{{$category->name}}</td>
                                            <?php
                                                if($category->is_activate == 1){$activate = '<span class="badge badge-info">active</span>';}
                                                ...
                                                <ul class="nav navbar-center navbar-top-links" style="border-radius: 15px;">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-user">
                                                            ...
                                                            <li class="divider"></li>
                                                            <li>
                                                                <button class="dropdown-item btn btn-priamry openActivationFrom" data-toggle="modal" data-target="#myModalActivation" data-id="{{$category->id}}">
                                                                    activation
                                                                </button>
                                                            </li>
                                                        </ul>
                        ...
                        <div style="margin-top: 20px; font-weight: 600; font-size: 16px;">
                            Showing 1 to <span id="showItems"></span> of <span>{{App\Models\Category::unArchive()->count()}}</span> entries
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/categories/delete'))}}" method="get">
                                        {{ csrf_field() }}
                    ...
                    <div class="modal fade" id="myModalActivation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form role="form" action="{{url(route('admin/categories/activate'))}}" method="get">
                                        {{ csrf_field() }}
                                        <p>Are You Sure To Update This Record ?</p>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on('click', '#load_more_button', function() {
                ...
                function load_data(_token) {
                    $.ajax({
                        url: `{{ route("admin/categories/pagination")}}/${offset}/${limit}`,
                        method: "POST",
                        data: {
                            _token: _token,
                        },
                        success: function(data) {
                            if (data.length > 0) {
                                for (let i = 0; i < data.length; i++) {
                                    ...
                                    edit_route =  "{{ route('admin/categories/edit') }}" + '/' + data[i].id;
                                    records += `
                                        <tr>
                                            <td>${data[i].id}</td>
                                            <td>${data[i].name}</td>
        ...
        $(document).on('keyup', '.data_search', function() {
            var q = $(this).val();
            var urlPath = "{{ route('admin/categories/search') }}";
